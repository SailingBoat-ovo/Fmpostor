...
namespace Fmpostor.Server.Plugins
{
    ...
    {
        private readonly FmpostorPluginAttribute _attribute;

        public PluginInformation(IPluginStartup? startup, Type pluginType)
        {
            _attribute = pluginType.GetCustomAttribute<FmpostorPluginAttribute>()!;
            ...
            Dependencies = pluginType.GetCustomAttributes<FmpostorDependencyAttribute>().Select(t => new DependencyInformation(t)).ToList();
            Startup = startup;
