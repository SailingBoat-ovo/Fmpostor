...
using Fmpostor.Api.Net.Messages.S2C;
    ...
    internal class Client : ClientBase
    {
        ...
        public override async ValueTask HandleMessageAsync(IMessageReader reader, MessageType messageType)
        {
            ...
            switch (flag)
            {
                ...
                {
                    Message01JoinGameC2S.Deserialize(reader, out var gameCode, out var productUserId, out var friendCode);

                    if (!string.IsNullOrEmpty(productUserId))
                    {
                        ProductUserId = productUserId;
                        Items["ProductUserId"] = productUserId;
                    }

                    if (!string.IsNullOrEmpty(friendCode))
                    {
                        FriendCode = friendCode;
                        Items["FriendCode"] = friendCode;
                    }
