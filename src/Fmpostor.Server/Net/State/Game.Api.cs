...
using Fmpostor.Api.Net.Messages;
using Impostor.Hazel;
using Fmpostor.Server.Events;
using Fmpostor.Server.Net.Inner;
...
namespace Fmpostor.Server.Net.State
{
    internal partial class Game : IGame
    {
        ...
        public async ValueTask SyncSettingsAsync()
        {
            ...
            {
                throw new FmpostorException("Attempted to change settings when the host was not spawned.");
            }

            if (GameNet.GameManager == null)
            {
                throw new FmpostorException("Attempted to change options when the game manager was not spawned.");
            }

            var gameOptionsTag = GameNet.GameManager.GetGameLogicTag(GameNet.GameManager.LogicOptions);
            if (gameOptionsTag == -1)
            {
                throw new FmpostorException("Attempted to change options when the LogicOptions was not spawned.");
            }
