...
using CSharpPoet;
using Fmpostor.Api.Innersloth.Generator.Generators;
using Microsoft.CodeAnalysis;

namespace Fmpostor.Api.Innersloth.Generator;
...
public sealed class SourceGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(filesProvider, (spc, files) =>
        {
            if (files.IsEmpty)
            {
                throw new InvalidOperationException($"No json files found in Fmpostor.Api/{DataPath}");
            }

            var enumGenerator = new EnumGenerator(spc, files);

            enumGenerator.Generate("ColorType", "Fmpostor.Api.Innersloth.Customization");
            ...
            enumGenerator.Generate("RpcCalls", "Fmpostor.Api.Net.Inner", underlyingType: CSharpEnumUnderlyingType.Byte);
